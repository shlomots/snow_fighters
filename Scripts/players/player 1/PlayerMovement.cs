...
public class PlayerMovement : MonoBehaviour
{
    ...
    IEnumerator OnTriggerEnter2D(Collider2D collider2D)
    {
        ...
        if (collider2D.gameObject.CompareTag("shield coin"))
        {
            ...
            int xPos;
            int yPos;
            FindObjectOfType<GameManager>().p1Protected = true;
            xPos = Random.Range(-26, -22);
            yPos = -16;
            yield return new WaitForSeconds(10);
            FindObjectOfType<GameManager>().p1Protected = false;
            collider2D.transform.position = new Vector2(xPos, yPos);//make coin reapear
        }
        ...
        if (collider2D.gameObject.CompareTag("super shot coin"))
        {
            ...
            int xPos;
            int yPos;
            FindObjectOfType<GameManager>().p1HasSuperShot = true;
            xPos = Random.Range(8, 15);
            yPos = 1;
            yield return new WaitForSeconds(10);
            FindObjectOfType<GameManager>().p1HasSuperShot = false;
            collider2D.transform.position = new Vector2(xPos, yPos);//make coin reapear
        }
